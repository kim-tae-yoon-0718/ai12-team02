...
def test_version_txt_extraction_table_key_is_read_as_table_axis(tmp_path):
    """[2026-09-04] 후보 VERSION.txt 의 `extraction_table:` 은 6-자산 ③ table 축이다.
    예전에는 이 키를 아예 읽지 않아 후보 추출표 버전이 provenance 에서 사라졌고,
    같은 키가 두 번 적혀도 아무도 몰랐다."""
    from grader.versioning import read_versions
    p = tmp_path / "VERSION.txt"
    p.write_text("evalset: v1-candidate.5\nextraction_table: v3.1-candidate.1\n"
                 "note_extraction_table: 후보(공식 v3 미변경)\ncorpus: v2\n", encoding="utf-8")
    v = read_versions(p)
    assert v.get("table") == "v3.1-candidate.1"
    assert v.get("evalset") == "v1-candidate.5"
