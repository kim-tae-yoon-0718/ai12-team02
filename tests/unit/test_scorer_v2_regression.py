...
def test_retrieval_na_for_non_search_route():
    ...
    # [2026-09-04] 검색 단계(retrieval_k/reranker_k/context_k)만 N/A 다.
    #   출처 좌표 채점은 비검색 경로에서도 계속한다 — 추출표·identity 로 답해도
    #   "그 값이 어느 문서 어느 좌표에서 나왔는가"는 여전히 채점 대상이기 때문이다.
    #   (이전 기대값 applicable is False 는 비검색 경로 문항이 근거를 아무렇게나
    #    붙여도 무감점이 되는 구멍이었다.) 단언은 약화가 아니라 강화됐다 —
    #   applicable 뿐 아니라 실제 점수까지 확인한다.
    cit = grade_citation(it, resp, non_search_routes=nsr)
    assert cit["applicable"] is True
    assert cit["search_used"] is False          # 검색 미사용은 기록으로 남는다
    assert cit["score"] == 0.0 and cit["n_citations"] == 0   # 출처 미표기 → 0점
    good = _r(answer="x", route="extract_table",
              citations=[Location(document="A", section="S", ref_no="S · 문단 1", line=10)])
    assert grade_citation(it, good, non_search_routes=nsr)["matched"] is True
    # 청크 검색 경로면 정상 채점
...
def test_document_not_found_is_never_an_absence_answer():
    """[2026-09-04 확정 정책] '확인할 수 없습니다'는 **모른다**는 뜻이지 '없다'가 아니다.

    예전 단언은 토글을 켜면 이 답이 1.0 이 된다고 고정했는데, 그것이 팀장 2-5
    (부재와 확인 실패를 합치지 말 것)와 충돌해 폐기됐다. 토글 값과 무관하게 0 이어야
    하고, 반대로 부재를 **단정한** 표현은 토글이 켜진 기본값에서 1.0 이어야 한다.
    """
    it = _it(answer_type="value", answer_raw="없음", answer_source="table")
    not_verified = _r(answer="문서에서 확인할 수 없습니다.", abstained=True)
    assert grade_short_answer(it, not_verified, {"accept_natural_absence_phrasing": False}).score == 0.0
    assert grade_short_answer(it, not_verified, {"accept_natural_absence_phrasing": True}).score == 0.0
    confirmed = _r(answer="해당 사항 없음", abstained=False)
    assert grade_short_answer(it, confirmed, {"accept_natural_absence_phrasing": True}).score == 1.0
    assert grade_short_answer(it, confirmed, {}).score == 1.0     # 기본값이 확정 정책
