...
def tool_decision(**overrides):
    observations = (("O1",)
                    if overrides.get("result_assessment", "initial") != "initial"
                    else ())
...
class FakeAgent:
    ...
    def decide(self, question, observations, session_document_id=None,
               locked_task_type=None, locked_answer_mode=None,
...
class FakeDualAgent(FakeAgent):
    ...
    def adjudicate_initial_plans(
        self, question, candidate_a, candidate_b, observation_a,
        observation_b, session_document_id=None,
    ):
        self.calls += 1
...
class TestDecisionContract:
    ...
    def test_verifier_approve_cannot_change_the_proposed_plan(self):
        ...
        approved = parse_verification(
            raw_verification(raw_decision()), proposed,
        ...
        with pytest.raises(Stage2AgentError, match="approve"):
            parse_verification(
                raw_verification(changed), proposed,
                valid_document_ids={"RFP-000001"})
    ...
    def test_verifier_correction_must_be_a_real_valid_change(self):
        ...
        proposed = tool_decision(
            ...
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[{"field": "컨소시엄 요건", "operator": "consortium_required",
                         "value": None, "negated": False,
        ...
        corrected_raw = raw_decision(
            ...
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[{"field": "컨소시엄 요건", "operator": "status_value_present",
                         "value": None, "negated": False,
                         "question_span": "공동수급 요건이 적힌"}],
        )
        corrected = parse_verification(
            raw_verification(
                corrected_raw, outcome="correct",
                issue_types=["condition_semantics"],
                verification_note="기재 여부 질문을 필수 여부로 해석했습니다."),
            proposed, valid_document_ids={"RFP-000001"})
        assert corrected.verified_plan.tool_plan.conditions[0].operator == \
            "status_value_present"
        with pytest.raises(Stage2AgentError, match="실제로 수정"):
            parse_verification(
                raw_verification(
                    proposed.as_dict(), outcome="correct",
                    issue_types=["condition_semantics"]),
                proposed, valid_document_ids={"RFP-000001"})
    ...
    def test_structural_comparison_ignores_order_notes_and_question_spans(self):
        ...
        first = tool_decision(
            action="table_select", task_type="select",
            document_scope="all_population", document_ids=[],
            requested_fields=["예산", "지역제한"], answer_mode="document_set",
            decision_note="첫 해석",
            conditions=[
                {"field": "예산", "operator": ">=", "value": 500000000,
                 "negated": False, "question_span": "5억원 이상"},
                {"field": "지역제한", "operator": "no_restriction", "value": None,
                 "negated": False, "question_span": "지역 제한 없는"},
            ],
        )
        second = tool_decision(
            action="table_select", task_type="select",
            document_scope="all_population", document_ids=[],
            requested_fields=["지역제한", "예산"], answer_mode="document_set",
            decision_note="다른 표현",
            conditions=[
                {"field": "지역제한", "operator": "no_restriction", "value": None,
                 "negated": False, "question_span": "지역제한이 없는"},
                {"field": "예산", "operator": ">=", "value": 500000000,
                 "negated": False, "question_span": "예산 조건"},
            ],
        )
    ...
    def test_adjudication_contract_never_creates_a_third_plan(self):
        ...
        assert schema["properties"]["selected_candidate"]["enum"] == [
            "candidate_a", "candidate_b", "clarify"]
    ...
    def test_requested_fields_are_canonicalized_without_reading_text(self):
        decision = tool_decision(
            requested_fields=["필수 제출 서류", "필수 제출 서류"])
    ...
    def test_empty_unanswerable_text_uses_fixed_transport_message(self):
        decision = parse(raw_decision(
            action="finalize", result_assessment="sufficient",
            answer_mode="unanswerable", final_answer="", final_items=[],
            document_ids=[], requested_fields=[],
            used_observation_ids=["O1"], used_evidence_ids=[],
        ), observations=("O1",), evidence=())
    ...
    def test_clarify_after_insufficient_result_is_a_valid_terminal_choice(self):
        decision = parse(raw_decision(
            action="clarify", result_assessment="insufficient",
            answer_mode="list", clarification="원문을 더 찾아볼까요?",
            final_answer="", final_items=[], used_observation_ids=["O1"],
            used_evidence_ids=[],
        ), observations=("O1",), evidence=())
    ...
    def test_list_text_in_wrong_strict_slot_is_canonicalized_to_one_item(self):
        decision = parse(raw_decision(
            action="finalize", result_assessment="sufficient",
            answer_mode="list", final_answer="원문에 해당 항목이 없습니다.",
            final_items=[], document_ids=[], requested_fields=[],
            used_observation_ids=["O1"], used_evidence_ids=["O1:E1"],
        ), observations=("O1",), evidence=("O1:E1",))
    ...
    def test_followup_schema_locks_the_initial_task_type(self):
        ...
        schema = decision_json_schema(
            ["RFP-000001"], ["O1"], ["O1:E1"], locked_task_type="extract",
    ...
    def test_schema_can_force_terminal_action_after_complete_tool_result(self):
        from stage2_agent import decision_json_schema
        schema = decision_json_schema(
            ["RFP-000001"], ["O1"], [], locked_task_type="select",
            locked_answer_mode="document_set", allowed_actions=("finalize", "clarify"))
    ...
    def test_finalize_rejects_unknown_result_or_evidence(self, key, value):
        ...
        raw = raw_decision(
            action="finalize", result_assessment="sufficient",
            document_ids=[], requested_fields=[],
            final_items=["A"], used_observation_ids=["O1"],
            used_evidence_ids=["O1:E1"],
        )
        raw[key] = value
        with pytest.raises(Stage2AgentError):
    ...
    def test_finalize_requires_an_observed_result(self):
        ...
        with pytest.raises(Stage2AgentError):
            parse(raw_decision(
                action="finalize", document_ids=[], requested_fields=[],
                final_items=["A"],
            ))

    def test_catalog_based_unanswerable_can_finish_without_a_tool(self):
        decision = parse(raw_decision(
            action="finalize", task_type="qa", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="unanswerable",
            final_answer="공식 문서 목록에서 찾을 수 없습니다.", final_items=[],
    ...
    def test_insufficient_result_can_end_as_unanswerable(self):
        decision = parse(raw_decision(
            action="finalize", result_assessment="insufficient",
            document_ids=[], requested_fields=[], answer_mode="unanswerable",
            final_answer="근거가 부족해 답할 수 없습니다.", final_items=[],
            used_observation_ids=["O1"], used_evidence_ids=[],
        ), observations=("O1",), evidence=())
        assert decision.answer_mode == "unanswerable"
    ...
    def test_clarify_ignores_inert_strict_output_slots(self):
        decision = parse(raw_decision(
            action="clarify", document_ids=[], requested_fields=[],
            clarification="어느 문서인가요?", final_answer="사용되지 않는 값",
        ))
        assert decision.action == "clarify"
    ...
    def test_document_set_ignores_inert_final_text(self):
        decision = parse(raw_decision(
            action="finalize", result_assessment="sufficient",
            task_type="select", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="document_set",
            final_answer="다시 쓰지 않는 설명", final_items=["다시 쓰지 않는 목록"],
            used_observation_ids=["O1"], used_evidence_ids=[],
        ), observations=("O1",), evidence=())
    ...
    def test_table_select_sanitizes_non_executable_slots(self):
        decision = parse(raw_decision(
            ...
            conditions=[{"field": "지역제한", "operator": "has_restriction",
                         "value": None, "negated": False, "question_span": "지역 제한"}],
        ))
    ...
    def test_status_operator_cannot_silently_ignore_a_search_value(self):
        ...
        with pytest.raises(Stage2AgentError, match="value는 null"):
            parse(raw_decision(
                action="table_select", task_type="select",
                document_scope="all_population", document_ids=[],
                requested_fields=["사업 개요"], answer_mode="document_set",
                conditions=[{
                    "field": "사업 개요", "operator": "status_value_present",
                    "value": "존재하지 않는 사업명", "negated": False,
    ...
    def test_openai_call_receives_observations_and_strict_dynamic_schema(
        ...
        class Completions:
            def create(self, **kwargs):
                ...
                message = SimpleNamespace(content=json.dumps(raw, ensure_ascii=False), refusal=None)
        ...
        observations = [{
            "observation_id": "O1", "tool": "table_lookup", "answer_text": "A",
...
class TestBoundedLoop:
    def test_dual_interpreters_agree_then_execute_once(
        self, table_rows, identity_index,
    ):
        import answer_pipeline as module
        from stage2_agent import Stage2Adjudication

        condition = {"field": "예산", "operator": "status_value_present",
                     "value": None, "negated": False, "question_span": "예산이 적힌"}
        candidate_a = tool_decision(
            action="table_select", task_type="select",
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[condition], decision_note="A")
        candidate_b = tool_decision(
            action="table_select", task_type="select",
            document_scope="all_population", document_ids=[],
            requested_fields=["예산"], answer_mode="document_set",
            conditions=[dict(condition, question_span="예산 항목이 있는")],
            decision_note="B")
        final = parse(raw_decision(
            action="finalize", result_assessment="sufficient",
            task_type="select", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="document_set",
            final_answer="", final_items=[],
            used_observation_ids=["O1"], used_evidence_ids=[],
        ), observations=("O1",), evidence=())
        agent = FakeDualAgent(
            candidate_b,
            Stage2Adjudication("candidate_a"),
            candidate_a, final)
        result = module.answer(
            "예산이 적힌 공고를 모두 찾아줘", None,
            lambda: None, lambda: None, table_rows,
    ...
    def test_dual_disagreement_executes_both_then_uses_selected_result(
        ...
        wrong = tool_decision(
            ...
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[{"field": "지역제한", "operator": "no_restriction",
                         "value": None, "negated": False,
        ...
        correct = tool_decision(
            ...
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[{"field": "지역제한", "operator": "status_value_present",
                         "value": None, "negated": False,
                         "question_span": "지역제한 항목이 적힌"}],
        )
        final = parse(raw_decision(
        ...
        assert result.execution_plan["initial_adjudication"][
            "selected_candidate"] == "candidate_b"
    ...
    def test_independent_verifier_corrects_initial_semantics_before_any_tool_runs(
        ...
        verification = parse_verification(
            ...
            wrong, valid_document_ids={"RFP-000001", "RFP-000002", "RFP-000003"})
        final = parse(raw_decision(
            action="finalize", result_assessment="sufficient",
            task_type="select", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="document_set",
        ...
        assert result.selected_document_ids == ["RFP-000002"]
        assert result.execution_plan["tool_calls"] == 1
        assert result.execution_plan["initial_plan_proposal"]["conditions"][0][
            "operator"] == "no_restriction"
        assert result.execution_plan["initial_plan_verification"]["outcome"] == "correct"
        assert result.execution_plan["decisions"][0]["conditions"][0][
    ...
    def test_table_selection_observation_is_compact(self):
        ...
        plan = SimpleNamespace(
            ...
            search_query="", answer_mode="document_set")
    ...
    def test_lookup_then_search_then_finalize_uses_only_declared_evidence(
        ...
        search = tool_decision(
            action="vector_search", task_type="extract", requested_fields=[],
            search_query="필수 제출 서류 전체", answer_mode="list",
        ...
        ), observations=("O1", "O2"), evidence=("O1:E1", "O2:E1"))
        ...
        result = module.answer(
            "자유로운 자연어 질문", small_store, get_embed, lambda: None, table_rows,
            {"routing_method": "llm_structgpt", "top_k": 2,
             "stage2_max_tool_calls": 3, "extraction_version": "v3",
             "schema_version": "x", "corpus": "v2", "chunking_version": "v3"},
            identity=identity_index,
            registry_scope=scope("RFP-000001", "RFP-000002", "RFP-000003"),
            get_stage2_agent=lambda: agent,
        )
        ...
        assert [d["action"] for d in result.execution_plan["decisions"]] == [
            "table_lookup", "vector_search", "finalize"]
    ...
    def test_table_selection_is_deterministic_and_not_rewritten(
        ...
        final = parse(raw_decision(
            ...
            task_type="select", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="document_set",
            final_items=[], used_observation_ids=["O1"], used_evidence_ids=[],
        ), observations=("O1",), evidence=("O1:E1",))
        agent = FakeAgent(select, final)
        result = module.answer(
            "질문", None, lambda: None, lambda: None, table_rows,
            {"routing_method": "llm_structgpt", "stage2_max_tool_calls": 3,
             "deadline_filter_default": {"select": False},
             "extraction_version": "v3", "schema_version": "x", "corpus": "v2"},
            identity=identity_index,
            registry_scope=scope("RFP-000001", "RFP-000002", "RFP-000003"),
    ...
    def test_existing_followup_turn_can_revise_a_wrong_initial_plan_once(
        ...
        wrong_select = tool_decision(
            ...
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set",
            conditions=[{"field": "사업 개요", "operator": "status_value_present",
                         "value": None, "negated": False,
                         "question_span": "사업이 있는지"}],
        )
        correction = parse(raw_decision(
            action="finalize", result_assessment="plan_wrong",
            task_type="qa", document_scope="all_population",
            document_ids=[], requested_fields=[], answer_mode="unanswerable",
            final_answer="공식 문서 목록에서 찾을 수 없습니다.", final_items=[],
            used_observation_ids=[], used_evidence_ids=[],
        ), observations=("O1",), evidence=())
        ...
        result = module.answer(
            "존재하지 않는 특정 사업이 있어?", None, lambda: None, lambda: None,
            table_rows,
            {"routing_method": "llm_structgpt", "stage2_max_tool_calls": 3,
             "deadline_filter_default": {"select": False},
             "extraction_version": "v3", "schema_version": "x", "corpus": "v2"},
    ...
    def test_sufficient_repeated_structured_action_does_not_execute_twice(
        ...
        import answer_pipeline as module
        condition = {"field": "예산", "operator": "status_value_present",
                     "value": None, "negated": False, "question_span": "예산"}
        select = tool_decision(
            action="table_select", task_type="select",
            document_scope="all_population", document_ids=[],
            requested_fields=[], answer_mode="document_set", conditions=[condition])
        sufficient = tool_decision(
            action="table_select", result_assessment="sufficient",
            task_type="select", document_scope="all_population",
            document_ids=["RFP-000001"], requested_fields=["예산"],
            answer_mode="document_set", conditions=[condition],
            used_observation_ids=["O1"])
        agent = FakeAgent(select, sufficient)
        result = module.answer(
            "예산이 적힌 공고", None, lambda: None, lambda: None, table_rows,
            {"routing_method": "llm_structgpt", "stage2_max_tool_calls": 3,
             "deadline_filter_default": {"select": False},
             "extraction_version": "v3", "schema_version": "x", "corpus": "v2"},
            identity=identity_index,
    ...
    def test_duplicate_tool_call_is_blocked_without_fallback(
        ...
        result = module.answer(
            ...
            {"routing_method": "llm_structgpt", "stage2_max_tool_calls": 3,
             "extraction_version": "v3", "schema_version": "x", "corpus": "v2"},
            identity=identity_index, registry_scope=scope("RFP-000001"),
            get_stage2_agent=lambda: agent,
        )
    ...
    def test_task_type_drift_is_blocked(self, table_rows, identity_index):
        ...
        first = tool_decision()
        drift = tool_decision(
            action="vector_search", task_type="qa", requested_fields=[],
            search_query="근거", answer_mode="summary",
            result_assessment="insufficient",
        )
    ...
    def test_tool_budget_is_enforced(self, table_rows, identity_index, small_store, fake_clients):
        import answer_pipeline as module
        _, _, get_embed, _ = fake_clients
        lookup = tool_decision()
        search = tool_decision(
            action="vector_search", requested_fields=[], search_query="근거",
            result_assessment="insufficient",
        )
        agent = FakeAgent(lookup, search)
        result = module.answer(
            "질문", small_store, get_embed, lambda: None, table_rows,
            {"routing_method": "llm_structgpt", "stage2_max_tool_calls": 1,
             "top_k": 2, "extraction_version": "v3", "schema_version": "x",
             "corpus": "v2", "chunking_version": "v3"},
            identity=identity_index, registry_scope=scope("RFP-000001"),
            get_stage2_agent=lambda: agent,
        )
        assert result.error_stage == "stage2_structgpt"
    ...
    def test_agent_failure_never_falls_back_to_old_router(self, monkeypatch, table_rows):
        import answer_pipeline as module
        ...
        result = module.answer(
            "질문", None, lambda: None, lambda: None, table_rows,
            {"routing_method": "llm_structgpt"}, get_stage2_agent=lambda: Broken(),
        )
        assert result.error_stage == "stage2_structgpt"
        assert result.route is None
    ...
    def test_baseline_and_stage1_routes_remain_separate(self, monkeypatch, table_rows):
        ...
        result = module.answer(
            "안녕", None, lambda: None, lambda: None, table_rows,
