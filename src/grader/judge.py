...
class Judge:
    ...
    def provider_name(self) -> str:
        return getattr(self.provider, "name", type(self.provider).__name__ if self.provider else "none")

    @property
    def is_mock(self) -> bool:
        """더미 provider 인가 — 이름이 아니라 provider 가 스스로 밝힌 값을 본다."""
        if self.provider is None:
            return False
        return getattr(self.provider, "usable_for_final", True) is False

    def assert_ready(self, generator_family: str | None = None,
                     strict: bool | None = None, *, mode: str | None = None,
                     config=None, prompt_hashes: dict | None = None) -> list[str]:
        """준비 상태를 점검한다. 판정 자체는 readiness.evaluate() 한 곳에서 한다.

        strict=None(기본): tier=='final' 인 심판만 막고, 그 외에는 보고만 한다.
        strict=True      : tier 와 무관하게 막는다.
        strict=False     : 무조건 보고만 한다.

        ★mode 를 안 주면 '최종 실행 여부'는 판정하지 않는다(설정만 검사).
          최종 실행 게이트는 mode="final" 과 strict=True 로 부른다.
        """
        from .readiness import evaluate

        r = evaluate(self, mode=(mode if mode is not None else "final"),
                     generator_family=generator_family, config=config,
                     prompt_hashes=prompt_hashes or getattr(self, "prompt_hashes", None))
        problems = [x for x in r.problems if not x.startswith("실행 모드가 final")] \
            if mode is None else list(r.problems)
        block = strict if strict is not None else (self.cfg.tier == "final")
        if problems and block:
