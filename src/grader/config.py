...
class RetrievalConfig:
    ...
    retrieval_k: int = 20
    # None = 리랭커 미사용. 재정렬 단계를 N/A 로 기록하고 점수를 만들지 않는다.
    reranker_k: int | None = 10
    context_k: int = 5
    ...
    # ★기본값도 실제 라우터 문자열로(HR eda55d6 채택). 예전 영문 추측값은 grader.yaml
    #   이 덮어써 왔지만, yaml 없이 로드하면 route 구분이 통째로 꺼졌다.
    non_search_routes: frozenset[str] = frozenset({
        "추출테이블_문서선별", "추출테이블_값조회", "추출테이블_비교조립",
        "identity_v2_값조회", "애매_되묻기", "검색불필요_인사응답", "검색불필요_사용법안내",
    })
    # ★clarification 판정 1차 신호(HR 2026-09-04 채택). response.route 가 여기 있으면
    #   텍스트 패턴보다 우선한다 — 모델이 실제로 이 값을 채워 보낸다.
...
class GradingConfig:
    ...
    accept_natural_absence_phrasing: bool = True   # 확정 정책 2026-09-04: 확정적 부재만 인정
...
def load_config(path: str | Path = "config/grader.yaml",
    ...
    return GraderConfig(
        ...
        retrieval=RetrievalConfig(
            ...
            # reranker_k 가 null/없음이면 "리랭커 미사용" — 숫자로 강제하지 않는다.
            reranker_k=(None if retrieval_raw.get("reranker_k", 10) is None
                        else int(retrieval_raw["reranker_k"])),
        ...
        grading=GradingConfig(
            ...
            accept_natural_absence_phrasing=bool(
                grading_raw.get("accept_natural_absence_phrasing", True)),
        ),
        gate=GateConfig(
