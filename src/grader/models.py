...
class Evidence(BaseModel):
    """근거 계약(4-근거) — 출처별로 형태가 다른 근거를 한 스키마로 담는다.

    ★왜 Location 만으로 부족한가:
      - 추출표가 "이 문서에 그 항목이 없다"(status=field_absent)를 근거로 삼을 때는
        가리킬 원문 줄이 **없다**. 없는 좌표를 지어내면 근거가 거짓이 된다.
      - identity(마감일 등)는 문서 본문이 아니라 등록부 CSV 가 출처다.
      - 같은 사실이 추출표 행으로도, 청크 좌표로도 가리켜질 수 있다. 어느 경로로
        인용했든 같은 원문을 가리키면 같은 근거로 인정해야 한다(4-근거 정규화).
    """

    model_config = ConfigDict(extra="allow")

    kind: Literal["chunk", "extraction_table", "identity"]
    document: str
    field: str | None = None          # 추출표·identity 근거의 필드명
    status: str | None = None         # 추출표 status (field_absent 등)
    value: str | None = None          # identity 값
    source: str | None = None         # chunks_v3 / extraction_table_v3 / identity_v2
    location: "Location | None" = None  # 원문 위치가 실제로 있을 때만
...
class EvaluationItem(BaseModel):
    ...
    # 4-근거: 정답을 뒷받침하는 근거 목록. location(청크 좌표)만으로 표현할 수 없는
    # 근거(추출표 미기재 행·identity 값)를 담는다. 있으면 출처 채점이 이걸 쓴다.
    evidence: list[Evidence] = Field(default_factory=list)
    # 평가셋에서 고칠 수 없는 상위(청킹·추출) 결함 때문에 정상 근거를 붙일 수 없는
    # 문항. 조용히 방치하지 않고 사유를 적어 담당자 확인 대기로 드러낸다.
...
class ModelResponse(BaseModel):
    ...
    id: str
    # ★4-5: 시스템 실패 응답은 answer 를 못 낸다. answer=null 이 통째로 로딩을
    #   중단시키면 "실패 1건 때문에 50문항 채점 전체가 안 돌아가는" 상태가 된다.
    #   null 을 허용하되 failure 사유를 반드시 함께 요구한다(아래 검증기).
    ...
    # ★[2026-09-04 §8] 인용은 두 형태를 모두 받는다.
    #   ① 원문 좌표 인용(Location) — 실제 줄이 있는 값에 그대로 쓴다(기존과 동일).
    #   ② Evidence 형 인용 — 추출표 미기재(status=field_absent)처럼 **가리킬 원문 줄이
    #      없는** 근거. kind/document/field/status/source 만 적고 location 은 생략한다.
    #   예전에는 위치 객체만 받아서, 모델이 미기재 근거를 정직하게 낼 방법이 없었다
    #   (빈 section 과 가짜 ref_no 를 넣어 위치처럼 위장하는 수밖에 없었다).
    citations: list[Location | Evidence] = Field(default_factory=list)  # 3-4-3 출처
    ...
    def _coerce_citations(cls, v):
        """인용 하나하나를 좌표형/Evidence형 중 **맞는 쪽**으로 읽는다.

        판별은 스키마가 요구하는 키로 한다 — ref_no·section 이 있으면 좌표 인용,
        없고 kind 가 있으면 Evidence 형 인용이다. 둘 다 아니면 원래 값을 그대로 두어
        pydantic 이 오류를 내게 한다(조용히 넘기지 않는다).
        기존 응답 파일은 전부 좌표 인용이므로 그대로 읽힌다(하위 호환).
        """
        if not isinstance(v, list):
            return v
        out = []
        for c in v:
            if isinstance(c, dict) and "ref_no" not in c and "kind" in c:
                out.append(Evidence.model_validate(c))
            else:
                out.append(c)
        return out

    @model_validator(mode="after")
    def _null_answer_requires_failure(self):
        """answer=null 은 failure 사유와 함께일 때만 허용한다.

        사유 없는 null 은 "실패인지 빈 답인지" 구분이 안 되고, 그대로 통과시키면
        시스템 실패가 조용히 오답 0점으로 섞인다 — 4-5 가 갈라놓으라고 한 두 가지다.
        """
        if self.answer is None and not (self.failure or "").strip():
            raise ValueError(
                f"id={self.id}: answer=null 은 failure 사유와 함께여야 한다 "
                f"(시스템 실패 0점 vs 오답 0점 구분, 4-5). "
                f"빈 답변이면 answer=\"\" 로 내라."
            )
        return self

    @property
    def answer_text(self) -> str:
        """채점기가 문자열로 다룰 때 쓰는 안전 접근자 — 실패 응답(None)은 빈 문자열."""
