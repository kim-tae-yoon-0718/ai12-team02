...
def main_metrics(results: list[dict], retrieval_agg: dict | None = None,
    ...
    if qa:
        ...
        out["primary"]["qa"] = {
            # ★[2026-09-04 정정] 이 값은 task_scoring 이 낸 **내용 점수**이지
            #   judge_faithfulness 점수가 아니다. '충실성'이라고 적어 두면 심판을
            #   돌리지 않은 실행에서도 충실성 숫자가 있는 것처럼 읽힌다.
            "metric": "내용 점수(task content score) — 검색 재현율(context_k)과 짝으로 읽을 것(D7)",
            "value": qa["score"], "n": qa["n"],
            "context_recall": ctx.get("recall@k"),
            "faithfulness_note": "충실성은 별도 축(report.faithfulness)이며 실심판을 "
                                 "돌린 실행에서만 값이 있다",
        }
...
def full_report(results: list[dict], retrieval_agg=None, extraction_agg=None,
    ...
    return {
        ...
        "failures": _failure_split(results),
    }


def _failure_split(results: list[dict]) -> dict:
    """0점을 **시스템 실패**와 **오답**으로 갈라 센다(4-5 / 6-1).

    한 칸에 합치면 "모델이 틀렸다"와 "실행이 깨졌다"가 같은 숫자가 되어 처방을
    못 고른다 — 전자는 모델 개선, 후자는 인프라 수정이다.
    """
    n = len(results)
    sys_fail = [r for r in results if (r.get("failure") or "").strip()
                or r.get("final_status") == "FAIL-system"]
    zero = [r for r in results if float(r.get("score") or 0.0) == 0.0]
    # id 가 없는 행(가중치 계산용 축약 행 등)도 들어올 수 있다 — 위치로 식별한다.
    sys_keys = {id(r) for r in sys_fail}
    content_zero = [r for r in zero if id(r) not in sys_keys]
    return {
        "n_items": n,
        "system_error_zero": len(sys_fail),
        "system_error_ids": sorted(r["id"] for r in sys_fail if r.get("id")),
        "content_zero": len(content_zero),
        "content_zero_ids": sorted(r["id"] for r in content_zero if r.get("id")),
        "system_error_rate": (len(sys_fail) / n) if n else 0.0,
        "note": "system_error_zero=실행 실패로 인한 0점(인프라), content_zero=모델이 "
                "틀린 0점. 둘 다 분모에는 남는다 — 빼면 실패할수록 점수가 오른다(4-5).",
    }
