...
def run(root: Path, *args, explicit_location_choice=True):
    script = root / "repo" / "src" / "chunking" / "build_chunks.py"
    script.parent.mkdir(parents=True, exist_ok=True)
    if not script.exists():
        shutil.copy(SCRIPT, script)
    # Windows 기본 콘솔(cp949)에서도 진단문에 쓰는 긴 대시·한글 때문에
    # 자식 프로세스가 중간에 죽지 않도록 UTF-8 모드로 고정한다.
    env = dict(os.environ, RAG_ROOT=str(root), PYTHONUTF8="1")
    cmd = [sys.executable, str(script), *args]
    # 대부분의 기존 시험은 location 자체가 아니라 다른 안전장치를 본다.
    # 새 기본 정책을 우회하지 않고, 참조본이 없는 가짜 최초 생성임을 명시한다.
    if (explicit_location_choice and "--verify-location" not in args
            and "--allow-unverified-location" not in args):
        cmd.extend(["--allow-unverified-location", "--unverified-location-reason",
                    "test fixture initial build"])
...
def test_location_check_requires_explicit_choice():
    """옵션을 깜빡한 청킹은 조용히 생성되지 않아야 한다."""
    root = with_fixture()
    try:
        r = run(root, explicit_location_choice=False)
        _check(r.returncode == 1, f"location 선택 없이 통과했다: {r.returncode}")
        _check("location 대조 방식" in r.stderr, f"중단 이유가 불명확하다: {r.stderr}")
        out = root / "shared_data" / "processed" / "chunks_v1"
        _check(not (out / "chunks.jsonl").exists(), "중단했는데 청크가 생성됐다")
    finally:
        shutil.rmtree(root, ignore_errors=True)


def test_unverified_location_requires_and_records_reason():
    """최초 생성 예외는 이유 없이 쓸 수 없고, 사용하면 산출물에 남아야 한다."""
    root = with_fixture()
    try:
        r = run(root, "--allow-unverified-location", explicit_location_choice=False)
        _check(r.returncode == 1, "이유 없는 location 예외가 통과했다")
        _check("unverified-location-reason" in r.stderr, r.stderr)

        reason = "initial baseline has no previous chunks"
        r = run(root, "--allow-unverified-location", "--unverified-location-reason", reason,
                explicit_location_choice=False)
        _check(r.returncode == 0, f"이유를 적은 최초 생성이 실패했다: {r.stderr}")
        stats = json.loads((root / "shared_data" / "processed" / "chunks_v1" /
                            "stats.json").read_text(encoding="utf-8"))
        lv = stats["location_verification"]
        _check(lv == {"result": "skipped_explicit", "reason": reason},
               f"예외 이유 기록이 다르다: {lv}")
    finally:
        shutil.rmtree(root, ignore_errors=True)
...
# 내용 없는 2절 제목이 3절 본문으로 흘러가면 검색 결과의 의미가 오염된다.
STRUCTURAL_LEAK_DOC = (
    "# 1. 개요\n\n"
    "첫 문단입니다.\n\n"
    "## 2. 배경\n\n"
...
def test_empty_heading_never_contaminates_next_sibling_chunk():
    """빈 구조 절의 제목은 다음 형제 절의 검색 텍스트에 들어가면 안 된다."""
    root = with_fixture()
    try:
        _rewrite_doc(root, 1, STRUCTURAL_LEAK_DOC)
        _check(run(root).returncode == 0, "정상 실행이 실패했다")
        rows = [json.loads(l) for l in
                (root.joinpath(*CHUNKS) / "chunks.jsonl").read_text(
                    encoding="utf-8").splitlines()]
        target = [r for r in rows if r["document_id"] == "RFP-000001"
                  and "과업 범위의 실제 내용" in r["search_text"]]
        _check(len(target) == 1, f"대상 청크가 하나가 아니다: {len(target)}")
        text = target[0]["search_text"]
        _check("2. 배경" not in text and "2.1 세부" not in text,
               f"관계없는 앞 절 제목이 다음 청크에 섞였다: {text}")
        stats = json.loads((root.joinpath(*CHUNKS) / "stats.json").read_text(
            encoding="utf-8"))
        _check(stats["heading_recovery"]["heading_dropped_cross_section"] == 2,
               f"차단 건수 기록이 다르다: {stats['heading_recovery']}")
    finally:
        shutil.rmtree(root, ignore_errors=True)
