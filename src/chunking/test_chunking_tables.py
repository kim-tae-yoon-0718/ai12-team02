...
from build_chunks import (                                    # noqa: E402
    ...
    find_lost_headings, find_tables, heading_has_inline_value, outer_cells,
    prune_pending_at_heading, split_blocks, split_table,
...
def test_empty_structural_heading_does_not_cross_into_sibling_section():
    """내용 없는 구조 제목은 다음 형제 절의 본문에 섞이면 안 된다."""
    pending = [("2. 배경", 2, False), ("2.1 세부", 3, False)]
    kept, dropped = prune_pending_at_heading(pending, 2)  # 다음 형제 ``3. 범위``
    _check(kept == [], f"관계없는 제목이 다음 절로 넘어갔다: {kept}")
    _check(dropped == 2, f"차단 건수가 2가 아니다: {dropped}")


def test_inline_fact_headings_stay_with_consecutive_value_list():
    """``항목: 값`` 제목은 같은 레벨의 연속 목록 안에서만 다음 unit까지 간다."""
    for text in ("□ 사업명 : 통합 정보시스템", "사업기간：12개월", "예산=5억원"):
        _check(heading_has_inline_value(text), f"독립 사실을 찾지 못했다: {text}")
    for text in ("2. 추진배경", "과업 범위", "참고사항:"):
        _check(not heading_has_inline_value(text), f"구조 제목을 사실로 봤다: {text}")

    pending = [("□ 사업명 : 통합 정보시스템", 3, True)]
    kept, dropped = prune_pending_at_heading(pending, 3)
    _check(kept == pending and dropped == 0, "같은 레벨의 값 목록을 끊었다")

    kept, dropped = prune_pending_at_heading(pending, 2)
    _check(kept == [] and dropped == 1, "상위 절을 넘어 값 제목을 운반했다")
